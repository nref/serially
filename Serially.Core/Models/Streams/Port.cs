...
using Serially.Core.Models.CommApi;
...
namespace Serially.Core.Models.Streams
{
  ...
  public class Port : IDisposable
  {
    ...
    private readonly DCB dcb = new DCB();
    private DetailedPortSettings portSettings;

    private Thread eventThread;
    private readonly ManualResetEvent threadStarted = new ManualResetEvent(false);

    private IntPtr closeEvent;
    private readonly string closeEventName = "CloseEvent";

    private int rts = 0;
    private readonly bool rtsavail = false;
    private int dtr = 0;
    private readonly bool dtravail = false;
    private int brk = 0;
    private int setir = 0;
    ...
    public bool Open()
    {
      ...
      if (CommAPI.FullFramework)
      {
        ...
        var o = new Models.Windows.Overlapped();
        txOverlapped = Kernel32.LocalAlloc(0x40, Marshal.SizeOf(o));
        o.Offset = 0;
        o.OffsetHigh = 0;
      ...
      if (hPort == (IntPtr)CommAPI.INVALID_HANDLE_VALUE)
      {
        ...
        throw new CommPortException($"CreateFile Failed: {Enum.GetName(typeof(APIErrors), e)} ({e})");
      }
      ...
      CommTimeouts ct = new CommTimeouts
      {
        ...
        ReadIntervalTimeout = uint.MaxValue, // this = 0xffffffff
    ...
    public bool Close()
    {
      ...
      {
        Kernel32.LocalFree(txOverlapped);
    ...
    public byte[] Output
    {
      set
      {
        ...
        if (value.GetLength(0) > sthreshold)
        {
          ...
          m_CommAPI.WriteFile(hPort, value, value.GetLength(0), ref written, txOverlapped);
        }
        else
        {
          // copy it to the tx buffer
          value.CopyTo(txBuffer, ptxBuffer);
          ptxBuffer += value.Length;
    ...
    /// </summary>
    public bool DTRAvailable => dtravail;
    ...
    /// </summary>
    public bool RTSAvailable => rtsavail;
    ...
    {
      get => portSettings.BasicSettings;
      set
      {
    ...
    private void CommEventThread()
    {
      ...
      if (CommAPI.FullFramework)
      {
        ...
        var o = new Models.Windows.Overlapped();
        rxOverlapped = Kernel32.LocalAlloc(0x40, Marshal.SizeOf(o));
        o.Offset = 0;
        o.OffsetHigh = 0;
      ...
      catch (Exception e)
      {
        if (rxOverlapped != IntPtr.Zero)
          Kernel32.LocalFree(rxOverlapped);
